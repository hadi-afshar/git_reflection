...
\And
Justin Domke\\
Australia's Information and Communications \\Technology Research Centre of Excellence\\
(NICTA), Canberra, ACT \\
...
$L$, number of leapfrog steps; $\epsilon$, leapfrog step size
}
\Output{next sample}
\BlankLine
{
...
\# \emph{Half-step evolution of momentum}:\\
$\bvec{p} \eq \bvec{p} - \epsilon  \nabla U(\bvec{q}) / 2$ 
%\hspace*{\fill} \# first \emph{momentum step}.
...
\# \emph{Full-step evolution of position}:\\
\If {\textsc{BaselineHMC}}{$\bvec{q} \eq \bvec{q} + \epsilon \bvec{p}$}
\Else {
\# (i.e.\ if \textsc{ReflectiveHMC}):\\
$t_0 \eq 0$
...
$\bvec{p} \eq - \bvec{p}$\hspace*{\fill} \# \emph{not required in practice; just for the proof of reversibility}\\
...
$t_x$, the time it is visited; $\Delta U$ = $U(\bvec{x}^+)$ - $U(\bvec{x}^-)$ and $\phi$, the visited partition boundary.  
